<template>
  <div class="desktop-reports">
    ...
    <div class="photo-cards pa-0 mt-10 d-flex flex-wrap" style="justify-content:space-between;">
      <template v-for="(card, index) of reports" :key="index">
        <component
          :is="report?.advert ? 'BaseAdvertisement' : 'BaseReportCard'"
          :report="card"
          width="416"
        ></component>
      </template>
      <!-- <component v-for="(card, index) of advertisements" :key="index"
        :is="advertisements.length > 0 && [3, 8, 9].includes(index) ? 'BaseAdvertisement' : 'BaseReportCard'"
      ></component> -->
    </div>

    <div class="d-flex justify-center pa-0 mt-10">
