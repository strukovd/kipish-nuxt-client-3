<template>
  <div class="establishment-mobile mmid3">
    <!-- BREADCRUMBS -->
    ...
    <section v-if="model.latitude && model.longitude" class="map">
      ...
      <div class="map">
        <YandexMap v-model="map" ref="map" :settings="{ location: { center: [ model.latitude, model.longitude ], zoom: 16, }, }" width="100%" height="270px">
          <YandexMapDefaultSchemeLayer :settings="{ theme: appStore.isDark ? 'dark' : 'light' }"/>
  ...
  methods: {
    ...
    fetch() {
      ...
      this.$http2.get(`/establishments/${this.appStore.sourceId}`)
        .then(r => {
          const payload: any = r.data.content[0];
